...
def train_DAAE(encoder,decoder,discriminator, train_loader):
    ...
    for epoch in range(max_epoch):
        ...
        for step, (image, label) in enumerate(train_loader):
            ...
            #discriminator
            encoder.eval()
            Z_real=Variable(torch.randn(image.size()[0],2)*5.)
    ...
    plt.title("losses for 20 epochs with latent size of 2, DAAE linear and BCE loss")
    fig.savefig("losses for 20 epochs with latent size of 2, DAAE linear and BCE loss")
...
if __name__ == '__main__':
    ...
    encoder=Q_net(28*28,1000,2)
    decoder=P_net(28*28,1000,2)
    discriminator=D_net(500,2)
    #encoder=Q_net()
    #decoder=P_net()
    ...
    #train_CNN(encoder,decoder,discriminator,train_loader,gen_lr,max_epoch,noise,latent_code_size,tiny,reg_lr)
    
    pickle.dump(encoder,open("2_encoder_DAAELINEAR_BCE_20","wb"))
    pickle.dump(decoder,open("2_decoder_DAAELINEAR_BCE_20","wb"))
    
    loaded_encoder=pickle.load(open("2_encoder_DAAELINEAR_BCE_20","r"))
    loaded_decoder=pickle.load(open("2_decoder_DAAELINEAR_BCE_20","r"))
    ...
    figure.savefig("2_original_DAAELINEAR_BCE_20")
    plt.close()
    ...
    figure.savefig("2_decoded_DAAELINEAR_BCE_20")
    plt.close()
