...
def train_CNN(encoder,decoder,discriminator,train_loader,gen_lr,max_epoch,noise,latent_code_size,tiny,reg_lr):
    #
    ...
    for epoch in range(max_epoch):
        for image,label in train_loader:
            ...
            recon_loss=criterion(X_sample.resize(100,784)+tiny, image+tiny)
            recon_loss.backward()
        ...
        print('epoch',epoch,'| reconstruction loss ', recon_loss.data[0],'| discriminator loss ', D_loss.data[0])
    ...
    plt.plot(range(max_epoch),BCEloss,label="MSE reconstruction loss")
    plt.xlabel("Number of epochs")
    plt.ylabel("losses")
    plt.gca().legend(('discriminator loss','MSE reconstruction loss'))
    plt.title("losses for 20 epochs with latent size of 32, total CNN and MSE loss")
    fig.savefig("losses for 20 epochs with latent size of 32, total CNN and MSE loss")
