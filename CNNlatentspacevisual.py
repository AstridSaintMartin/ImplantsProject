...
xtest.resize(xtest.shape[0],1,xtest.shape[1],xtest.shape[2])
xtest=Variable(torch.from_numpy(xtest))

#Take latent space
encoder=pickle.load(open("encoder_totalCNN_32","r"))
decoder=pickle.load(open("decoder_totalCNN_32","r"))
xtest=encoder.forward(xtest.type(torch.FloatTensor))
xtest=xtest.data.numpy()
xtest=np.resize(xtest,(10000,32))
